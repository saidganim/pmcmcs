...
__global__ void iteration(double* temp_init, double* temp_tmp, double* conductivity, int N, int M, double* maxdiff) {
        unsigned i = blockIdx.x * blockDim.x * loop_size + threadIdx.x;
        unsigned j = i % M + 1;
...
// execute kernel
                ...
                iteration<<<(N * M / loop_size / threadBlockSize + 1), threadBlockSize>>>(deviceA, deviceB, deviceConductivity, N, M, deviceMaxdiff);
